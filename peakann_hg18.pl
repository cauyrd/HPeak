...
#### .\
#### ..\
#### data\hg18\refFlat.out
#### data\hg18\phastCons\
####                   chr1.out
####                   chr2.out
####                   ...
#### data\hg18\chromFa\     ###available at genome.ucsc
####                 chr1.fa
####                 chr2.fa
####                 ...
#### peakann_hg18.pl
...
sub hitsLookUp {
	...
	my $gene1=do { local(@ARGV, $/) = "$dir\/data\/hg18\/refFlat.out"; <>};
	$gene1=~s/\r//g;
	my @genes=split /\n/,$gene1;
	my @new_genes=();
	my %lfs=();
...
#			open(FH,"<$dir\/data\/hg18\/phastCons\/$chr_o.out") || die "PhastCons file error: $!\n";
#			$pre_chr=$chr;
#		}
...
#sub getSeq {
#	my ($chr,$start,$end)=@_;
#	open(IN1,"<$dir\/data\/hg18\/chromFa\/$chr\.fa") || die "can not open chromosome seq file\n";
