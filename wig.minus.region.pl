...
$argc = @ARGV;
$argc == 6 || die "Provide species, input region, input window hits count file names (case, control), output WIG file name and the window size 
(default is 25).\n";
...
$controlwindowhitscountname = $ARGV[3];
$outputwigname = $ARGV[4];
$windowsize = $ARGV[5];
