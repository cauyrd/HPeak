...
for($k=1; $k<($nchr+1); $k++)
{
	if ($sp1 eq "human" || "hg19" || "hg18")
	{
		$com = "$dir/chiphmm ".$name.".select.chr".$k.".txt ".$name.".paras.txt ".$name.".hmmout.chr".$k.".txt $windowSize ".$sizes[$k-1];
	}
	elsif ($sp1 eq "mouse" || "mm9")
	{
		$com = "$dir/chiphmm_mm9 ".$name.".select.chr".$k.".txt ".$name.".paras.txt ".$name.".hmmout.chr".$k.".txt $windowSize ".$sizes[$k-1];
	}
##	$com = "$dir/chiphmm ".$name.".select.chr".$k.".txt ".$name.".paras.txt ".$name.".hmmout.chr".$k.".txt $windowSize ".$sizes[$k-1];
	print "$com\n";
	system($com);
        system("cat poisson.out >>allinone");
        system("rm poisson.out");

}
$com = "perl $dir/autoregion.pl 1 $sp1 $siglevel $windowSize ".$name.".paras.txt ".$name.".hmmout ".$name;
print "$com\n";
system($com);
